...
export async function main(ns) {
  ...
  const maxPlayerServers = ns.getPurchasedServerLimit()
  ...
  while (true) {
    ...
    let action = purchasedServers.length < maxPlayerServers ? settings.actions.BUY : settings.actions.UPGRADE
    ...
    if (action == settings.actions.BUY) {
      ...
      if (targetRam === settings.minGbRam) {
        while (ns.getServerMoneyAvailable('home') * settings.totalMoneyAllocation >= ns.getPurchasedServerCost(targetRam) * maxPlayerServers) {
          targetRam *= 2
        }
      ...
      targetRam = Math.min(targetRam, maxGbRam)
      ...
      if (smallestCurrentServer === maxGbRam) {
